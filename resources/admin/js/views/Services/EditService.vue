...
const save = (e) => {
    ...
    let service = {
        ...
        is_publish: isPublish.value ? 1 : 0,
        price: price.value,
        is_price_from: isPriceFrom.value  ? 1 : 0,
        is_price_individual: isPriceIndividual.value  ? 1 : 0,
    }


    api.put(`/api/service/${route.params.id}`, service).then((res) => {
        console.log(res)
        alert("Сохранено")
        location.reload()
    })
...
const fetchService = () => {
    api.get(`/api/service/${route.params.id}`).then((res) => {
        console.log(res)
        ...
        isPriceFrom.value = Boolean(res.data.is_price_from)
        isPriceIndividual.value = Boolean(res.data.is_price_individual)
    })
}
